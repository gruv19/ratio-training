...
const articleCardemplate = (content, type) => {
  ...
  ${(type === 'default') ? `<a class="article-card__link" href="${base}blog/article/${id}">` : ''}
    <img src="${image}" alt="" class="article-card__img">
  ${(type === 'default') ? `</a>` : ''}
  <div class="article-card__content">
    <p class="article-card__tag">${tag}</p>
    <h1 class="article-card__title"><a class="article-card__link" href="${base}blog/article/${id}">${title}</a></h1>
