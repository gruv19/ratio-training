const paginationTemplate = (prevArticle, nextArticle) => {
  ...
  return `<div class="pagination${(prevId === -1) ? ' pagination--without-prev' : ''}${(nextId === -1) ? ' pagination--without-next' : ''}">
    ${(prevId > -1) ? `<div class="paginamtion__prev">
        <a class="pagintation__arrow pagintation__arrow--prev" href="/blog/article/${prevId}">
        ...
        <div class="pagination__label">
          Go back: <a href="/blog/article/${prevId}" class="pagination__article-title">${prevTitle}</a>
        </div>
      </div>` : ''}
    ${(nextId > -1) ? `<div class="pagination__next">
        <div class="pagination__label">
          Next up: <a href="/blog/article/${(nextId > -1)}" class="pagination__article-title">${nextTitle}</a>
        </div>
        <a class="pagintation__arrow pagintation__arrow--next" href="/blog/article/${nextId}">
